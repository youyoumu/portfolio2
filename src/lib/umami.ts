...
interface Track {
  (eventName: string, obj?: EventData): void;
  d: typeof dTrack;
}

const _track = (eventName: string, obj?: EventData) => {
  if (typeof umami === "undefined") return;
  if (obj) {
    umami.track(eventName, obj).catch(() => {});
  } else {
    umami.track(eventName).catch(() => {});
  }
};
const dTrack = debounce(_track, 1000);

export const track: Track = Object.assign(_track, { d: dTrack });
